...
public class SitemapGenerationWithSitemapIndexExample
{
    ...
    /// <summary>
    /// This is a bogus interface defined in this example to simulate something you
    /// might use to get a list of URls from your CMS or something like that
    /// </summary>
    ...
    /// <summary>
    /// This is an example showing how you might take a large list of URLs of different kinds
    /// of resources and build both a bunch of sitemaps (depending on how many URls you have)
    /// as well as a sitemap index file to go with it
    /// </summary>
    public void GenerateSitemapsForMyEntireWebsite()
    {
        // Imagine you have an interface that can return a list of URLs for a resource that
        // you consider to be high priority -- for example, the product detail pages (PDPs) of your website
        var productPageUrlStrings = _websiteUrlRetriever.GetHighPriorityProductPageUrls();

        // Build a list of X.Web.Sitemap.Url objects and determine what is the appropriate ChangeFrequency,
        // TimeStamp (aka "LastMod" or date that the resource last had changes), and the a priority for the
        // page. If you can build in some logic to prioritize your pages then you are more sophisticated than most! :)
        var allUrls = productPageUrlStrings.Select(url => new Url
        {
            ...
            // In this case we don't know when the page was last modified so we wouldn't really set this.
            // Only assigning here to demonstrate that the property exists.
        ...
        // sitemap index files could be generated if we wanted
        ...
        {
            // It's up to you to figure out what the URI is to the sitemap you wrote to the file system.
            // In this case we are assuming that the directory above has files exposed via
            // the /sitemaps/ subfolder of www.mywebsite.com
        ...
        // After this runs you'll want to make sure your robots.txt has a reference to the sitemap index (at the
        // bottom of robots.txt) like this: "Sitemap: https://www.mywebsite.com/sitemaps/sitemap-index.xml"
        // You could do this manually (since this may never change) or if you are ultra-fancy, you could dynamically
        // update your robots.txt with the names of the sitemap index file(s) you generated
    }
